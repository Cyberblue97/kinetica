...
import type { DashboardStats, TodaySession, ExpiringPackage } from "@/types";
...
export default function DashboardPage() {
  ...
  const statCards = [
    ...
    {
      title: "이번 주 만료",
      value: stats?.expiring_packages_this_week ?? 0,
      icon: TrendingUp,
                ...
                {expiringPackages.map((mp) => (
                  ...
                  >
                    <div className="flex flex-col">
                      ...
                      <span className="text-xs text-slate-500">
                        {mp.package_name} · 만료:{" "}
                        {format(new Date(mp.expiry_date), "MM/dd", {
                          locale: ko,
                        })}
                      </span>
                    </div>
